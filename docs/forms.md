...
`openOnFocus` (по умолчанию `true`) управляет автооткрытием при входе в поиск кликом или с клавиатуры. `minSearchLength` (по умолчанию `0`) задаёт минимальную длину запроса без пробелов по краям для автоматического показа списка и загрузки удалённых опций. При очистке или сокращении запроса ниже порога список закрывается, а поле сохраняет фокус.

Стрелка справа и клавиши `ArrowDown`/`ArrowUp` открывают список явно, независимо от этих настроек, в том числе запускают загрузку для пустого запроса. Escape и стрелка закрытия скрывают только список: запрос и фокус сохраняются, последующий ввод снова проверяет порог. При выходе из Select запрос сбрасывается. Настройки применяются к одиночному и множественному `searchable` Select.

Портальный список привязан к слою ближайшего Modal/Drawer и расположен вне прокручиваемой панели. Список фонового Select остаётся под затемнением нового окна. `portal={false}` сохраняет рендер рядом с полем.
